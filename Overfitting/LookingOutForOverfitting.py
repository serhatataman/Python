# The program below uses the so called k-nearest neighbors algorithm.
# The idea is to not only look at the single nearest training data point (neighbor) but for example the five nearest points, if k=5.
# The normal nearest neighbor classifier amounts to using k=1.
#
...
#
# Hint: You can get the model accuracy using function knn.score.
# For example: knn.score(x_train, y_train) returns the training set accuracy
# after you have first created the classifier by calling knn.fit(x_train, y_train).
#
